...
```
implementation("com.spotify.confidence:openfeature-provider-android:0.6.11")
```

Where `0.6.11` is the most recent version of the Provider. Released versions can be found under "Releases" within this repository.
...
Applications can select a strategy based on the age of the cached resolve storage:

```kotlin
val storageStatus = confidence.getStorageStatus(
    MaxAgeStorageCheck(maxAgeMillis = TimeUnit.HOURS.toMillis(24))
)
val strategy = when (storageStatus) {
...
To try this flow locally, set `CLIENT_SECRET=<MY_SECRET>` in `local.properties` and run the `ConfidenceDemoApp` configuration. The demo uses a one-minute maximum age and displays both the resolve storage status and selected initialization strategy. Clear the app data for an empty-storage launch, restart within a minute for a fresh launch, or wait a minute before restarting for a stale launch.
...
```
implementation("com.spotify.confidence:confidence-sdk-android:0.6.11")
```

Where `0.6.11` is the most recent version of this SDK. 
